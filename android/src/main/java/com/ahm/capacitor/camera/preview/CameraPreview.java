...
public class CameraPreview extends Plugin implements CameraActivity.CameraPreviewListener {
    ...
    @PluginMethod
    public void listCamera(PluginCall call) {
        CameraManager cameraManager = (CameraManager) getContext().getSystemService(Context.CAMERA_SERVICE);
        JSArray cameraArray = new JSArray();
        try {
            for (String cameraId : cameraManager.getCameraIdList()) {
                ...
                if (lensFacing != null) {
                    String cameraType = (lensFacing == CameraCharacteristics.LENS_FACING_FRONT) ? "front" :
                                        (lensFacing == CameraCharacteristics.LENS_FACING_BACK) ? "rear" :
                                        "external";
                    JSObject cam = new JSObject();
                    cam.put("name", cameraType);
                    cam.put("id", cameraId);
                    cameraArray.put(cam);
                }
            }
            ...
            call.reject("Failed to access cameras", e);
        }
    }
